...
#+ATTR_REVEAL: :frag (appear)
-  An argument whose form /guarantees/ that, if
   the premises are true, the conclusion must also be true
...
#+ATTR_REVEAL: :frag (appear)
- An argument which is (i) valid and (ii) has true
  premises
...
-  Some bad argument forms nevertheless may initially appear to be good
   arguments
#+END_NOTES
...
#+BEGIN_QUOTE
  ...
  shouldn't believe anyting he says.
#+END_QUOTE
